 ...
 */
public class test extends AbstractionLayerAI {
    ...
    public test(UnitTypeTable a_utt) {
        this(a_utt, new AStarPathFinding());
    }

    public test(UnitTypeTable a_utt, PathFinding a_pf) {
        super(a_pf);
        reset(a_utt);
    }
...
//        System.out.println("LightRushAI for player " + player + " (cycle " + gs.getTime() + ")");
    ...
    public void barracksBehavior(Unit u, Player p, PhysicalGameState pgs) {
     ...
     // heavy > ranged > light   
       ...
       if (p.getResources() >= lightType.cost&&(nLight>=1||nHeavy>=1)) {
           if(mLight>=4)
           {
               if(mRanged<=2)
               {
      ...
      */        
        if (p.getResources() >= rangedType.cost&&nRanged>=1) {
           train(u, rangedType);
           return;
        }
        
        if (nworker>=4&&p.getResources() >= lightType.cost){
            train(u, lightType);
            return;
        }
        /*
        if (nworker>=4&&p.getResources() >= rangedType.cost){
            train(u, rangedType);
            return;
        }*/
...
//            System.out.println("LightRushAI.meleeUnitBehavior: " + u + " attacks " + closestEnemy);
    ...
    public void workersBehavior(List<Unit> workers, Player p, PhysicalGameState pgs, GameState gs) {
        ...
        if ((nbarracks == 0) && (!freeWorkers.isEmpty()) && nworkers > 1
            ...
            int resources = p.getResources();
            //Unit u = freeWorkers.remove(1);   
            //buildIfNotAlreadyBuilding(u,,u.getX(),u.getY(),reservedPositions,p,pgs);
            int base_x=0;
            int base_y=0;
            for (Unit u2 : pgs.getUnits()) 
            {
            ...
            {
                Unit u = freeWorkers.remove(1);  
                build(u, barracksType, (base_x) % pgs.getWidth(), (base_y-2) % pgs.getWidth());
            }
            else
            {
                Unit u = freeWorkers.remove(1); 
                build(u, barracksType, (base_x+2)% pgs.getWidth(), (base_y-1)% pgs.getWidth());
                //Unit u = freeWorkers.remove(0); 
                //build(u, barracksType, (base_x-2)% pgs.getWidth(), (base_y+2)% pgs.getWidth());
            }
